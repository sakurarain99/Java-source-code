 ...
 */
/*
    它代表一件事情，把一个Channel跟一个Selector

    每次当一个Channel被注册到一个selector到时候都会创建一个SelectionKey，一个key保持有效直到
    它被调用了cancel()方法取消，或关闭其Channel或关闭selector而被取消为止，取消一个键不会立即从
    Selector中移除而是将其添加到Selector的cancelled-key set(取消键集)中以便在下次selection operation.(选择操作)期间将其删除。

    SelectionKey包括两个操作集合是通过整数表示的
        一个是ready set(准备集合)  interest set(感兴趣的操作集合)
 */
public abstract class SelectionKey {
     ...
     */
    /*
        获取与这个SelectionKey所关联的Channel对象
     */
