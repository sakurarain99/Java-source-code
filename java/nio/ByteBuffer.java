    ...
    implements Comparable<ByteBuffer>
{
     ...
     */
    /*
        创建一个新的Buffer，其内容是此缓冲区的内容的共享子序列。

        这个新的Buffer的内容将会以这个Buffer的当前位置为起始位置，对这个Buffer的任何修改
        都对新的Buffer是可见的，反之亦然。两个Buffer的 position, limit, and mark 值将是独立的
        互不干扰的
     */
     ...
     */
    /*
        它会创建一个只读的ByteBuffer，共享这个Buffer的内容

        新Buffer的内容就是原来Buffer的内容，对Buffer的改变新Buffer也是可见的，
        然而新的Buffer是只读的不允许去修改里面的内容，对与两个Buffer的 position, limit, and mark
        都是独立的，互不干扰的
     */
